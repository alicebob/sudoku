        ...
        r =
            case msg of
                ...
                KeyPress k ->
                    case k of
                        'H' ->
                            moveSelect (\( x, y ) -> ( x - 1, y ))

                        'J' ->
                            moveSelect (\( x, y ) -> ( x, y + 1 ))
                        ...
                        _ ->
                            if Char.isDigit k then
                                case String.toInt <| String.fromChar k of
                                    Ok i ->
                                        setSelectedCell <| Grid.Guess i
...
doMove state ( x, y ) cell =
    let
        ...
        new =
            Grid.gridSet x y old cell

        -- drop the 'redo' future, if any
        his =
            Array.slice 0 (state.current + 1) state.history
    in
        { state
        ...
        rendCell x y c =
            ...
            in
                cell (stl ++ borderStl) <|
                    case c of
                        Grid.Clue n ->
                            Html.b [] <| [ text <| toString n ]

                        Grid.Guess n ->
                            div [ Events.onClick (Click ( x, y )) ] [ text <| toString n ]

                        Grid.Unknown ->
                            div [ Events.onClick (Click ( x, y )) ] [ nbsp ]

        cells =
            Grid.indexedMapGrid rendCell (current state)
    in
        divGrid [] cells
...
nbsp =
    Html.span [ Attr.property "innerHTML" (Json.Encode.string "&nbsp;") ] []
